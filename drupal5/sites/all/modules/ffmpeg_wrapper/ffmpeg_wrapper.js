// $Id: ffmpeg_wrapper.js,v 1.1.2.4 2008/12/17 00:15:13 arthuregg Exp $;
...
/**
 * File Data Display
 * This function catches the file choice on the testing page and displays
 * the file data. 
 * @NOTE Drupal 5 specific
 */
$(document).ready( function () {
  // select the current value of the checked radio
  $('input[@name=data]').bind('change', function() {
    $('#ffmpeg_wrapper_file_data_display').hide();
	$('#ffmpeg_wrapper_file_data_display').html('<h3>Loading file data... </h3>');
	$('#ffmpeg_wrapper_file_data_display').show('slow');  
    // build the URL
    var url = ffmpeg_wrapper_url+$(this).val();
    $.getJSON(url, function(data) {
       $('#ffmpeg_wrapper_file_data_display').hide('slow', function () {    	
         $('#ffmpeg_wrapper_file_data_display').html(data);
         $('#ffmpeg_wrapper_file_data_display').show('slow');
       });
    });
  });
